...
export const createwhislist = async ({
  ...
  try {
    if (!create_at || !update_at) {
      var query =
        "Insert into whishlist(user_id,product_id) Values($1,$2) returning *";

      var arr = [user_id, product_id];
    } else {
      var query =
        "Insert into whishlist(user_id,product_id,create_at,update_at) Values($1,$2,$3,$4) returning *";
...
export const updatewhislist = async ({ id, user_id, product_id }) => {
  try {
    const query =
      "Update whishlist set user_id=$1,product_id=$2 where id=$3 returning *";
